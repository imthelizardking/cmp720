...
void uart_init(void) {
    // TODO: Your code here
	/*USART_SR_TXE
	RCC_APB2ENR |= (1 << 4); /* set bit 4 to 1 */
	USART1_BRR = 0x2D9; /* set brr 84mhz%115200 hz */
	USART_SR_TXE |= (1 << 7);
	USART_CR1_UE |= (1 << 13);
	USART_CR1_TE |= (1 << 3);
	USART1_CR1 = USART_C1_UE | USART_C1_TE; /* diğerleri sıfır olsun diye or_eşittir */yapmadık */
    	RCC_APB2ENR |= (1 << 4);          // Enable USART1 clock (bit 4)
    	USART1_BRR = 0x2D9;               // Baud rate: 84MHz / 115200
    	USART1_CR1 = USART_CR1_UE | USART_CR1_TE;  // Enable USART + transmitter
}
...
void led_init(void) {
    // TODO: Your code here
	RCC_AHB1ENR |= (1 << 3);
	GPIOD_MODER &= ~(3 << (LED_GREEN_PIN * 2)); /* clear mask 1111...00...111 */
	GPIOD_MODER |= (1 << (LED_GREEN_PIN * 2)); /* set */
}
...
void led_toggle(void) {
    // TODO: Your code here
	GPIOD_ODR ^= (1 << LED_GREEN_PIN); /* xor the bit */
}
